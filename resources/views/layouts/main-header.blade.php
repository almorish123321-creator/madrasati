    ...
    <ul class="nav navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown ">
            <a class="nav-link top-nav" data-toggle="dropdown" href="#" role="button" aria-haspopup="true"
                aria-expanded="false">
                <i class="ti-bell"></i>
                <span class="badge badge-danger notification-status" id="unreadCount">{{ Auth::user()->unreadNotifications->count() }}</span>
            </a>
            <div class="dropdown-menu dropdown-menu-right dropdown-big dropdown-notifications">
                <div class="dropdown-header notifications d-flex justify-content-between">
                ...
                @forelse(Auth::user()->notifications()->take(5)->get() as $notification)
                    ...
                    <a href="{{ route('notifications.markRead', $notification->id) }}" class="dropdown-item {{ $notification->read_at ? 'text-muted' : 'font-weight-bold' }}">
                        <i class="fas {{ $data['icon'] ?? 'fa-bell' }} text-{{ $data['color'] ?? 'info' }}"></i>
                        {{ $data['title'] ?? 'إشعار' }}
                        <small class="float-left text-muted time">{{ $notification->created_at->diffForHumans() }}</small>
                    </a>
                @empty
                    <a href="#" class="dropdown-item text-muted text-center">لا توجد إشعارات</a>
                @endforelse
                <div class="dropdown-divider"></div>
                <a href="{{ route('notifications.index') }}" class="dropdown-item text-center text-primary">عرض جميع الإشعارات</a>
            </div>
        </li>
        <li class="nav-item dropdown ">
            <a class="nav-link top-nav" data-toggle="dropdown" href="#" role="button" aria-haspopup="true"
